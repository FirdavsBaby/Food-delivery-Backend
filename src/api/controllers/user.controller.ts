...
import { PaymentT } from "../../types/Payment.type.js";
import Stripe from "stripe";
import cfg from "../../config/cfg.js";
import User from "../../models/User.model.js";
import Purchase from "../../models/Purchase.model.js";
import { RateT } from "../../types/Rate.type.js";
import Joi from "joi";
import Rate from "../../models/Rate.model.js";

if (!cfg.payment_key) throw new CustomError("Type 'payment_key'", 400)
...
export const MinusCount = async(req:AuthRequest, res:Response, next:NextFunction):Promise<void> => {
    try {
        const {id : user_id} = req.user as UserT
        const {id: menu_id} = req.params
        const findItem = await Cart.findOne({where:{menu_id, user_id}})
        if (!findItem) throw new CustomError("Item not found", 400)
...
export const AddToCart = async(req:AuthRequest,res:Response,next:NextFunction):Promise<void> => {
    try {
        ...
        const findItem = await Cart.findOne({where:{menu_id, user_id}})
        if (findItem) {
            const {count} = findItem.dataValues as CartT
...
export const RemoveFromCart = async(req:AuthRequest, res:Response, next:NextFunction):Promise<void> => {
    try {
        ...
        if (!findItem) throw new CustomError("Cart item is not found", 400)
...
export const newPayment = async(req:AuthRequest, res:Response, next:NextFunction):Promise<void> => {
    try {
        const { amount, id } = req.body as PaymentT
        const {id: user_id} = req.user as UserT
...
export const newPurchase = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {balance, id} = req.user as UserT
        ...
        for (let i = 0; i < cart.length; i++) {
            ...
            if (price === 0) throw new CustomError("Not items in cart", 400)
            
            if (price > balance) throw new CustomError("Balnace is very low", 400)
...
export const AllPurchases = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {role} = req.user as UserT
        if (role !== "admin" && role !== "restoran") throw new CustomError("You have not permission to purchase", 400) 
...
export const QueuePurchase = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
...
export const MyPurchase = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {id} = req.user as UserT
...
export const confirmation = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {id} = req.params
        const {status} = req.body
        if (status !== "accepted" &&  status !== "denied") throw new CustomError("please select one of the statuses accepted or denied", 400)

        const findPurchase = await Purchase.findByPk(id)
        if (!findPurchase) throw new CustomError("Purchase not found", 400)
...
export const raiting = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {id} = req.params
        const {rate} = req.body as RateT
        const schema = Joi.number().required().min(1).max(5)
        const {error} = schema.validate(rate)
        if (error) throw new CustomError("Rate can be from 1 to 5 and rate is required", 400)
        const purchase = await Purchase.findByPk(id)
        if (!purchase) throw new CustomError("Puchase not found", 400)
        const findRate = await Rate.findOne({where: {purchase_id: id}}) 
        if (findRate) throw new CustomError("This purchase is already raited", 400)
...
export const PurchasesHistory = async(req:AuthRequest,res:Response,next:NextFunction): Promise<void> => {
    try {
        const {id} = req.user as UserT 
