...
export const run = async(app:Application): Promise<void> => {
    ...
    if (!checkAdmin) {
        if (!cfg.admin_password || !cfg.admin_email || !cfg.admin_username) return console.log("Please enter admin data to .env");
        const hashPassword: string = await bcrypt.hash(cfg.admin_password, 12)
